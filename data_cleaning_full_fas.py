...
def clean_fas_data():
    ...
    file_path = 'raw_data/FAS_full.csv'
    ...
    # Columns to delete
    delete_columns = [
        'FREQUENCY','SCALE', 'PRECISION', 'DECIMALS_DISPLAYED', 'INSTR_ASSET', 'FA_INDICATORS',
    ...
    data_filtered_ind = data_cleaned[data_cleaned['INDICATOR'].isin(selected_indicators)]
    ...
    # Renaming TIME_PERIOD to 'Year' and OBS_VALUE to 'value'
    data_filtered_ind.rename(columns={'TIME_PERIOD': 'Year'}, inplace=True)
    data_filtered_ind.rename(columns={'OBS_VALUE': 'Value'}, inplace=True)

    # Filter for the years 2004 to 2023
    data_filtered_ind = data_filtered_ind[(data_filtered_ind['Year'] >= 2004) & (data_filtered_ind['Year'] <= 2023)]
    
    # Sort the DataFrame by country, Indicator and year
    data_filtered_compl = data_filtered_ind.sort_values(by=["COUNTRY", "INDICATOR", "Year"])

    # Save the cleaned data to a new CSV file
    data_filtered_compl.to_csv('clean_data/FAS_full_cleaned.csv', index=False)
